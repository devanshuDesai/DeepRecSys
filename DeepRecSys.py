...
def DeepRecSys():
    ...
    if args.queue == True:
        ...
        for i in range(args.inference_engines):
            if (args.model_accel) and (i == (args.inference_engines - 1)):
                p = Process(target=accelInferenceEngine,
                            args=(args, accelRequestQueue, i,
                                  responseQueues[i], inferenceEngineReadyQueue))
            else:
                p = Process(target=inferenceEngine,
                            args=(args, requestQueue, i,
                                  responseQueues[i],inferenceEngineReadyQueue))
            p.daemon = True
            ...
            for i in range(args.inference_engines):
                if (responseQueues[i].qsize()):
                    ...
                    if response == None:
                        ...
                        print("Joined ", inference_engines_finished,
                              " inference engines")
                        sys.stdout.flush()
                    else:
                        key = (response.epoch, response.batch_id,
                               response.exp_packet)
                        if key in response_sets.keys():  # Response already in the list
                            ...
                            remain = curr_val[2]-1
                            response_sets[(response.epoch, response.batch_id, response.exp_packet)] = (
                                arr, inf, remain)
                        else:  # New response!
                        ...
                        if remain == 0:
                            ...
                            if len(response_latencies) % request_granularity == 0:
                                print("Running latency: ", np.percentile(
                                    response_latencies[int(-1 * request_granularity):], 95) * 1000.)
                                sys.stdout.flush()
                                # Add
                                pidQueue.put(np.percentile(
                                    response_latencies[int(-1 * request_granularity):], 95) * 1000.)
        ...
        print("Exiting DeepRecSys after printing ",
              len(responses_list), "/", agg_requests)
        ...
        meas_qps_responses = list(
            filter(lambda x: (not x['exp_packet']) and (x['sub_id'] == 0), responses_list))

        initial_time = meas_qps_responses[0]['inference_end_time']
        end_time = meas_qps_responses[-1]['inference_end_time']

        print("Measured QPS: ",  (len(meas_qps_responses)) /
              (end_time - initial_time))
        print("Measured p95 tail-latency: ",
              np.percentile(final_response_latencies, 95) * 1000., " ms")
        print("Measured p99 tail-latency: ",
              np.percentile(final_response_latencies, 99) * 1000., " ms")
